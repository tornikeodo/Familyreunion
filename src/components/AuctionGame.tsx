...
import { playBidSound, playSpinSound, playHammerSound, playTimerWarning, playTimerEnd, playResultReveal } from "@/lib/sounds";

const AUCTION_TIMER_SECONDS = 15;
const BID_COOLDOWN_SECONDS = 4;
...
export default function AuctionGame({ room, gamePlayers, isHost, playerId, onRefresh }: Props) {
  ...
  useEffect(() => {
    if (!room.auctionActive || !room.auctionStartedAt) {
      setTimeLeft(AUCTION_TIMER_SECONDS);
      return;
    }

    const update = () => {
      const elapsed = Math.floor((Date.now() - room.auctionStartedAt!) / 1000);
      const remaining = Math.max(0, AUCTION_TIMER_SECONDS - elapsed);
      setTimeLeft(remaining);

      if (remaining === 3) playTimerWarning();
      if (remaining === 0) playTimerEnd();
    };

    update();
    const interval = setInterval(update, 500);
    return () => clearInterval(interval);
  }, [room.auctionActive, room.auctionStartedAt]);

  // Bid cooldown timer
  useEffect(() => {
    if (!room.lastBidAt) { setCooldown(0); return; }

    const update = () => {
      const elapsed = (Date.now() - room.lastBidAt!) / 1000;
      const remaining = Math.max(0, BID_COOLDOWN_SECONDS - elapsed);
  ...
  // Budget warning
  useEffect(() => {
    if (currentPlayer && !currentPlayer.isHost && currentPlayer.budget < 100_000_000) {
  ...
  async function handleCloseAuction() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      setSpinResult(null); showNotif("sold! 🔨"); onRefresh();
    } catch (err) { showNotif(err instanceof Error ? err.message : "Failed"); }
    finally { setActionLoading(false); }
  }
  ...
  const canBid = !!(currentPlayer && !currentPlayer.isEliminated && !currentPlayer.isHost &&
    room.currentPosition && !alreadyHasPosition && room.auctionActive && cooldown === 0);
  ...
  const timerColor = timeLeft <= 3 ? "text-red-400" : timeLeft <= 7 ? "text-yellow-400" : "text-green-400";
    ...
    <div className="max-w-7xl mx-auto fade-in">
      ...
      {budgetWarning && currentPlayer && !currentPlayer.isHost && (
        <div className="glass p-3 mb-4 border-red-500/30 bg-red-500/5 flex items-center gap-3">
          ...
          <div>
            <span className="text-sm font-semibold text-red-400">low budget warning</span>
            <span className="text-xs text-white/40 ml-2">
              you have ${formatMoney(currentPlayer.budget)} left. spend carefully!
            </span>
          </div>
        </div>
      )}
            ...
            <div className="grid lg:grid-cols-5 gap-4">
              ...
              <div className="lg:col-span-3 glass p-6 auction-active">
                <div className="text-center mb-4">
                  ...
                  {/* Timer */}
                  <div className="mb-4">
                    <div className={`text-4xl font-black ${timerColor} transition-colors`}>{timeLeft}s</div>
                    <div className="w-48 h-1.5 bg-white/10 rounded-full mx-auto mt-1 overflow-hidden">
                      <div className={`h-full rounded-full transition-all duration-500 ${timeLeft <= 3 ? "bg-red-500" : timeLeft <= 7 ? "bg-yellow-500" : "bg-green-500"}`}
                        style={{ width: `${(timeLeft / AUCTION_TIMER_SECONDS) * 100}%` }} />
                    </div>
                    {timeLeft === 0 && isHost && <p className="text-xs text-red-400 mt-1">timer done! close the auction</p>}
                  </div>
                ...
                {/* Cooldown indicator */}
                {cooldown > 0 && !currentPlayer?.isHost && (
                  <div className="text-center mb-3">
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-orange-500/10 border border-orange-500/20">
                      ...
                      <span className="text-xs text-orange-400 font-semibold">cooldown: {cooldown}s</span>
                    </div>
                  </div>
                )}

                {/* Bid controls */}
                {currentPlayer && !currentPlayer.isHost && !currentPlayer.isEliminated && room.currentPosition && !alreadyHasPosition && (
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {quickBids.map(qb => (
                        <button key={qb.label} onClick={() => setBidAmount(qb.value.toString())}
                          ...
                          disabled={qb.value > (currentPlayer?.budget || 0) || cooldown > 0}>{qb.label}</button>
                      ))}
                    </div>
                    <div className="flex gap-2">
                      <input type="text" className="glass-input flex-1 text-lg font-bold" placeholder="your bid..."
                        ...
                        onKeyDown={e => e.key === "Enter" && canBid && handleBid()} disabled={cooldown > 0} />
                      <button onClick={handleBid} disabled={actionLoading || !canBid}
                        className="btn-gold px-6 text-lg font-black disabled:opacity-40">
                        {cooldown > 0 ? `${cooldown}s` : actionLoading ? "..." : "BID 🔥"}
                      </button>
                    </div>
                    {bidAmount && (
                      <div className="flex items-center justify-between text-xs text-white/40 px-1">
                        <span>bid: ${formatMoney(parseInt(bidAmount) || 0)}</span>
                        <span className={parseInt(bidAmount) > (currentPlayer.budget * 0.5) ? "text-red-400" : ""}>
                          left: ${formatMoney(currentPlayer.budget - (parseInt(bidAmount) || 0))}
                        </span>
                      </div>
                    )}
                  </div>
                ...
                {isHost && (
                  <div className="mt-6 text-center">
                    <button onClick={handleCloseAuction} disabled={actionLoading} className="btn-danger px-8 py-3 text-sm font-bold">
                      {actionLoading ? "closing..." : "🔨 SOLD! close auction"}
                    </button>
                  </div>
                )}
              </div>
