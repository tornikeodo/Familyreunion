...
export default function AuctionGame({ room, gamePlayers, isHost, playerId, onRefresh }: Props) {
  ...
  const auctionedCount = (room.auctionedPositions || []).length;
  const totalAuctions = POSITIONS.length * 3;
  ...
  async function handleFinishGame() {
    if (!confirm("are you sure you wanna end the game?")) return;
    try {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/40">
              ...
              progress: {auctionedCount}/{totalAuctions}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="w-32 h-2 bg-white/10 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${(auctionedCount / totalAuctions) * 100}%` }}
            />
          </div>
          <button
          ...
          {currentPlayer && (
            <div className="glass p-4">
              ...
              <div className="grid grid-cols-4 gap-1.5">
                {POSITIONS.map(pos => {
                  const hasPlayer = ((currentPlayer.lineup || {}) as Record<string, PlayerData>)[pos];
                  return (
                    <div
                      key={pos}
            ...
            <div className="glass p-12 text-center">
              <div className="mb-6">
                ...
                <p className="text-white/40 max-w-md mx-auto">
                  {isHost
                    ? "hit the button to randomly pick the next position & tier for auction"
                ...
                {currentAuctionPlayer && (
                  <div className={`player-card ${getCardTier(room.wheelResult.optionIndex)} max-w-xs mx-auto mb-6 p-6`}>
                    <div className="text-center">
                      <div className="text-6xl mb-3" style={{ filter: "drop-shadow(0 0 10px rgba(124,58,237,0.3))" }}>⚽</div>
                      ...
                      <div className="mt-2 inline-block px-2 py-0.5 rounded text-[10px] bg-white/10 text-white/40">
                        {room.wheelResult.position}
                      </div>
                    </div>
                  </div>
                )}
              ...
              {isHost ? (
                ...
                <div className="text-center">
                  <div className="inline-flex items-center gap-2 text-white/30 text-sm">
                    ...
                    host is setting the starting price...
                  </div>
                </div>
              )}
            </div>
