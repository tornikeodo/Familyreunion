...
export default function AuctionGame({ room, gamePlayers, isHost, playerId, onRefresh }: Props) {
  ...
  // 3-second countdown after last bid, then auto-close
  useEffect(() => {
    // Reset auto-close flag when auction state changes
    if (!room.auctionActive || !room.lastBidAt) {
      setCountdown(null);
    ...
    const tick = () => {
      ...
      const remaining = Math.max(0, BID_COUNTDOWN_SECONDS - elapsed);
      setCountdown(Math.ceil(remaining * 10) / 10); // Show 1 decimal

      if (remaining <= 0.5) playTimerWarning();

      // Auto-close: only the host triggers the close call to avoid duplicates
  ...
  const countdownDisplay = countdown !== null ? countdown : null;
  const countdownUrgent = countdownDisplay !== null && countdownDisplay <= 1;
      ...
      {budgetWarning && currentPlayer && !currentPlayer.isHost && (
        <div className="glass p-3 mb-4 border-red-500/30 bg-red-500/5 flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-xs text-white/40 ml-2">${formatMoney(currentPlayer.budget)} left</span>
          </div>
        </div>
      )}
            ...
            <div className="grid lg:grid-cols-5 gap-4">
              <div className="lg:col-span-3 glass p-6 auction-active">
                <div className="text-center mb-4">
                  ...
                  {/* 3-second countdown after a bid */}
                  {hasAnyBid && countdownDisplay !== null && (
                    <div className="mb-4">
                      <div className={`text-5xl font-black transition-colors ${
                        countdownUrgent ? "text-red-400 animate-pulse" : "text-yellow-400"
                      }`}>
                        {countdownDisplay <= 0 ? "SOLD!" : `${countdownDisplay.toFixed(1)}s`}
                      </div>
                      <div className="w-48 h-2 bg-white/10 rounded-full mx-auto mt-2 overflow-hidden">
                        <div className={`h-full rounded-full transition-all duration-100 ${
                          countdownUrgent ? "bg-red-500" : "bg-yellow-500"
                        }`} style={{ width: `${Math.max(0, (countdownDisplay / BID_COUNTDOWN_SECONDS) * 100)}%` }} />
                      </div>
                      <p className="text-xs text-white/30 mt-1">
                        {countdownDisplay <= 0 ? "auction closed!" : "bid now or it's gone!"}
                      </p>
                    </div>
                  )}
                  ...
                  {!hasAnyBid && (
                    <div className="mb-4">
                      ...
                      <p className="text-xs text-white/30 mt-1">3s countdown starts after the first bid</p>
                    </div>
                  )}
                ...
                {/* Bid controls */}
                {canBid && (countdownDisplay === null || countdownDisplay > 0) && (
                ...
                {/* Host can force-skip if nobody is bidding */}
                {isHost && !hasAnyBid && (
