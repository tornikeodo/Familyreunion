...
export default function AuctionGame({ room, gamePlayers, isHost, playerId, onRefresh }: Props) {
  ...
  // Calculate progress - how many positions filled across all active players
  const activePlayers = gamePlayers.filter(p => !p.isHost && !p.isEliminated);
  const totalPositionsNeeded = activePlayers.length * 11;
  const totalPositionsFilled = activePlayers.reduce((sum, p) => {
    return sum + Object.keys(p.lineup || {}).length;
  }, 0);
  ...
  const searchPlayers = useCallback(async (query: string, position: string) => {
  ...
  // Load players when wheel result changes
  useEffect(() => {
    if (room.wheelResult?.position && !room.auctionActive) {
      searchPlayers("", room.wheelResult.position);
      setSelectedPlayer(null);
      setSearchQuery("");
    }
  }, [room.wheelResult?.position, room.auctionActive, searchPlayers]);

  // Debounced search
  useEffect(() => {
    if (!room.wheelResult?.position || room.auctionActive) return;
    const timer = setTimeout(() => {
      searchPlayers(searchQuery, room.wheelResult!.position);
    }, 300);
    return () => clearTimeout(timer);
  }, [searchQuery, room.wheelResult, room.auctionActive, searchPlayers]);
  ...
  async function handleStartAuction() {
    if (!selectedPlayer) {
      showNotif("select a player first!");
      return;
    }
    const price = parseInt(startPrice.replace(/,/g, ""));
    if (isNaN(price) || price < 0) {
      showNotif("enter a valid starting price!");
      return;
    }
    ...
    try {
      const res = await fetch(`/api/rooms/${room.id}/auction`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "start", playerId, amount: price, selectedPlayer }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      setStartPrice("");
      ...
      showNotif(err instanceof Error ? err.message : "Failed");
    } finally {
  ...
  async function handleBid() {
    ...
    try {
      ...
      showNotif(err instanceof Error ? err.message : "Bid failed");
    } finally {
      setActionLoading(false);
    }
  }
  ...
  async function handleFine() {
    ...
    if (!fineTarget || isNaN(amount) || amount <= 0) {
      showNotif("pick a player and enter amount!");
      return;
    }
    try {
      ...
      showNotif(err instanceof Error ? err.message : "Failed to fine");
    }
  }
  ...
  // Get the currently auctioned player details
  ...
  return (
    <div className="max-w-7xl mx-auto fade-in">
      {/* Notification Toast */}
      {notification && (
        <div className="fixed top-4 right-4 z-50 glass-strong px-6 py-3 text-sm font-medium fade-in">
          {notification}
        </div>
      )}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/40">
              ...
              progress: {totalPositionsFilled}/{totalPositionsNeeded} positions filled
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="w-32 h-2 bg-white/10 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${(totalPositionsFilled / totalPositionsNeeded) * 100}%` }}
            />
          </div>
          <button
          ...
          {isHost && (
            <>
              <button
                ...
                className="glass-button px-3 py-2 text-xs text-red-400"
              >
                💰 fine
              </button>
              <button
      ...
      <div className="grid lg:grid-cols-12 gap-4">
        {/* Sidebar: Players & Budgets */}
        <div className="lg:col-span-3 space-y-4">
          <div className="glass p-4">
            <h3 className="text-xs font-bold text-white/40 uppercase tracking-wider mb-3">💰 budgets & progress</h3>
            <div className="space-y-2">
              {gamePlayers.map(p => {
                const filledPositions = Object.keys(p.lineup || {}).length;
                const isComplete = filledPositions >= 11;
                return (
                  <div
                    key={p.id}
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-1.5">
                        ...
                        {isComplete && !p.isHost && <span className="text-[9px] px-1.5 py-0.5 rounded bg-green-500/20 text-green-400">✓</span>}
                      </div>
                    ...
                    {!p.isHost && (
                      <>
                        <div className="flex items-center justify-between">
                          <span className={`text-sm font-bold ${p.budget < 100_000_000 ? "text-red-400" : "text-green-400"}`}>
                            ${formatMoney(p.budget)}
                          </span>
                          <span className="text-[10px] text-white/30">{filledPositions}/11</span>
                        </div>
                        <div className="w-full h-1 bg-white/5 rounded-full mt-1.5 overflow-hidden">
                          <div
                            className={`h-full rounded-full transition-all ${isComplete ? "bg-green-500" : "bg-gradient-to-r from-purple-500 to-blue-500"}`}
                            style={{ width: `${(filledPositions / 11) * 100}%` }}
                          />
                        </div>
                      </>
                    )}
                    ...
                    <div
                      ...
                      className={`text-center p-1.5 rounded-lg text-[10px] transition-all ${
                        hasPlayer
                          ...
                          ? "bg-purple-500/15 border border-purple-500/30 animate-pulse"
        ...
        <div className="lg:col-span-9 space-y-4">
          {/* IDLE STATE: Ready to spin */}
          {!room.auctionActive && !room.wheelResult && !spinning && (
            <div className="glass p-12 text-center">
              <div className="mb-6">
                <div className="text-7xl mb-4 inline-block" style={{ filter: "drop-shadow(0 0 20px rgba(124,58,237,0.3))" }}>
                  🎰
                </div>
                <h3 className="text-3xl font-black mb-2">ready to spin?</h3>
                <p className="text-white/40 max-w-md mx-auto">
                  {isHost
                    ? "hit the button to randomly pick the next position. game continues until everyone has 11 players!"
              ...
              {isHost && (
                <button
                  onClick={handleSpin}
                  className="btn-gold px-16 py-5 text-xl font-black hover:scale-105 transition-transform"
                >
                  🎡 SPIN THE WHEEL
                </button>
              )}
          ...
          {/* SPIN RESULT: Host picks a player and sets starting price */}
          {room.wheelResult && !room.auctionActive && !spinning && (
            <div className="glass p-6 auction-active">
              <div className="text-center mb-6">
                <p className="text-sm text-white/40 mb-4">position selected! 🎯</p>
                <div className="inline-block glass-strong px-8 py-4 rounded-xl mb-4">
                  <div className="text-[10px] text-white/30 uppercase tracking-wider">position</div>
                  <div className="text-3xl font-black text-purple-400">{room.wheelResult.position}</div>
                </div>
              </div>
              ...
              {isHost ? (
                <div className="grid md:grid-cols-2 gap-6">
                  {/* Player Search */}
                  <div>
                    <h4 className="text-sm font-bold text-white/60 mb-3">search & pick a player</h4>
                    <input
                      type="text"
                    ...
                    <div className="max-h-[300px] overflow-y-auto space-y-2">
                      {searchLoading && (
                        <div className="text-center py-4 text-white/30 text-sm">searching...</div>
                      )}
                      {!searchLoading && searchResults.length === 0 && (
                        <div className="text-center py-4 text-white/30 text-sm">no players found</div>
                      )}
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <div className="text-xs text-white/40">{p.team} • {p.nationality}</div>
                            </div>
                            <span className={`text-xl font-black ${getRatingColor(p.rating)}`}>
                              {p.rating}
                            </span>
                          </div>
                        </button>
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-white/60 mb-3">selected player</h4>
                    {selectedPlayer ? (
                      <div className="player-card gold p-6 mb-4">
                        <div className="text-center">
                          <div className="text-5xl mb-3">⚽</div>
                          <div className="inline-block px-3 py-1 rounded-lg bg-black/30 mb-2">
                            <span className={`text-3xl font-black ${getRatingColor(selectedPlayer.rating)}`}>
                              {selectedPlayer.rating}
                            </span>
                          </div>
                          <div className="text-xl font-bold mt-2">{selectedPlayer.name}</div>
                          <div className="text-white/50 text-sm">{selectedPlayer.team}</div>
                          <div className="text-white/30 text-xs mt-1">{selectedPlayer.nationality}</div>
                        </div>
                      </div>
                    ) : (
                      <div className="glass-strong p-8 text-center mb-4">
                        <div className="text-white/20">← select a player from the list</div>
                      </div>
                    )}

                    <div className="space-y-3">
                      <div>
                        <label className="text-xs text-white/40 mb-1 block">starting price</label>
                        <input
                          type="text"
                        ...
                        {startPrice && (
                          <p className="text-sm text-white/40 text-center mt-1">
                            = <span className="text-yellow-400 font-bold">${formatMoney(parseInt(startPrice) || 0)}</span>
                          </p>
                        )}
                      </div>
                      {/* Quick price buttons */}
                      <div className="flex gap-2 flex-wrap">
                        {[1_000_000, 5_000_000, 10_000_000, 25_000_000, 50_000_000, 100_000_000].map(v => (
                          <button
                            key={v}
                            onClick={() => setStartPrice(v.toString())}
                            className="glass-button px-3 py-1.5 text-xs flex-1"
                          >
                            ${formatMoney(v)}
                          </button>
                        ))}
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={handleSkip} disabled={actionLoading} className="glass-button px-6 py-3">
                          skip ⏭️
                        </button>
                      </div>
                    </div>
          ...
          {room.auctionActive && currentAuctionPlayer && (
            <div className="glass p-8 auction-active">
              <div className="text-center mb-6">
                ...
                {/* Player Card */}
                <div className="player-card gold max-w-xs mx-auto mb-6 p-6">
                  <div className="text-center">
                    <div className="text-5xl mb-3">⚽</div>
                    <div className="inline-block px-3 py-1 rounded-lg bg-black/30 mb-2">
                      <span className={`text-3xl font-black ${getRatingColor(currentAuctionPlayer.rating)}`}>
                        {currentAuctionPlayer.rating}
                      </span>
                    </div>
                ...
                <div className="glass-strong p-6 max-w-sm mx-auto mb-6 relative overflow-hidden">
                  ...
                  <div className="relative">
                    ...
                    <div className="text-4xl font-black text-yellow-400 mb-1">
                      ${formatMoney(room.currentHighestBid || 0)}
                    </div>
                    {highestBidderPlayer && (
                    ...
                    {!highestBidderPlayer && (
                      <div className="text-sm text-white/30">no bids yet</div>
                    )}
                  </div>
                </div>
              </div>
                ...
                <div className="max-w-sm mx-auto space-y-3">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {quickBids.map((qb) => (
                      <button
                        key={qb.label}
                        onClick={() => setBidAmount(qb.value.toString())}
                        className="glass-button px-2 py-2.5 text-xs font-medium disabled:opacity-20"
                        disabled={qb.value > (currentPlayer?.budget || 0)}
                      >
                        {qb.label}
                      </button>
                    ))}
                  ...
                  <div className="flex gap-2">
                    <input
                      type="text"
                      className="glass-input flex-1 text-lg font-bold"
                    ...
                    <button
                      ...
                      className="btn-gold px-8 text-lg font-black"
                    >
                      {actionLoading ? "..." : "BID 🔥"}
                    </button>
                  </div>
              ...
              {/* Can't bid messages */}
              {currentPlayer && room.currentPosition && alreadyHasPosition && (
                <p className="text-center text-yellow-400/60 text-sm mt-4">
                  you already have a {room.currentPosition}, can&apos;t bid on this one
                </p>
              )}
              ...
              {currentPlayer?.isHost && (
                <p className="text-center text-white/40 text-sm mt-4">
                  you&apos;re the host, you don&apos;t participate in bidding
                </p>
              )}
              ...
              {isHost && (
                <div className="mt-8 text-center">
                  <button
                    ...
                    disabled={actionLoading}
                    className="btn-danger px-10 py-4 text-lg font-bold"
                  >
            ...
            <div className="glass p-6">
              ...
              <div className="flex gap-2 mb-4 flex-wrap">
                {gamePlayers.filter(p => !p.isHost).map(p => (
                  <button
                    key={p.id}
                    onClick={() => setSelectedLineupPlayer(selectedLineupPlayer === p.id ? null : p.id)}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. 50000000"
                  value={fineAmount}
                  onChange={e => setFineAmount(e.target.value.replace(/[^0-9]/g, ""))}
                />
                {fineAmount && (
                  <p className="text-xs text-yellow-400 mt-1">${formatMoney(parseInt(fineAmount) || 0)}</p>
                )}
              </div>
              <div>
