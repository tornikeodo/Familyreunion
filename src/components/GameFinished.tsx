...
export default function GameFinished({ gamePlayers }: Props) {
  ...
  const [postMode, setPostMode] = useState<"results" | "tournament" | "manager">("results");
  ...
  if (postMode !== "results") {
    return (
      <div className="max-w-6xl mx-auto">
        <div className="mb-6">
          <button 
            onClick={() => setPostMode("results")}
            className="glass-button px-4 py-2 text-sm"
          >
            ← back to results
          </button>
        </div>
        {postMode === "tournament" && <Tournament gamePlayers={gamePlayers} />}
        {postMode === "manager" && <ManagerMode gamePlayers={gamePlayers} />}
      </div>
    );
  }
      ...
      <div className="glass p-8 text-center mb-6">
        ...
        {canRunTournament && (
          <div className="flex gap-3 justify-center flex-wrap">
            <button 
              onClick={() => setPostMode("tournament")}
              className="btn-gold px-6 py-3 text-base"
            >
              ⚽ tournament mode
            </button>
            <button 
              onClick={() => setPostMode("manager")}
