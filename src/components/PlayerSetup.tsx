...
interface Props {
  roomId: string;
  playerId: string;
  picksPerPosition: number;
  onComplete: () => void;
}

export default function PlayerSetup({ roomId, playerId, picksPerPosition, onComplete }: Props) {
  const PICKS_PER_POSITION = picksPerPosition;
  ...
  const tierLabels = Array.from({ length: PICKS_PER_POSITION }, (_, i) => 
    i === 0 ? "⭐ Best" : i === 1 ? "🥈 Second" : i === 2 ? "🥉 Third" : `#${i + 1}`
  );
  ...
  const tierColors = tierColorsList.slice(0, PICKS_PER_POSITION);
