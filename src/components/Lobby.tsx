...
export default function Lobby({ room, gamePlayers, isHost, onStartGame }: Props) {
  return (
      ...
      <div className="glass p-8 text-center">
        ...
        <div className="glass p-4 mb-6 text-left text-sm">
          ...
          <ul className="text-white/40 space-y-1 text-xs">
            <li>• each player starts with $1 billion budget</li>
            <li>• host spins to pick a position, then selects any player</li>
            <li>• everyone bids, highest bidder wins the player</li>
            <li>• game ends when all players have 11/11 positions filled</li>
            <li>• after auction, teams compete in a tournament!</li>
          </ul>
        </div>

        {isHost && gamePlayers.length >= 2 && (
          <button onClick={onStartGame} className="btn-primary w-full py-4 text-lg">
            start the auction! 🎯
          </button>
        )}
