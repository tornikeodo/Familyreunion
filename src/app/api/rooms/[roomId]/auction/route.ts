  ...
  try {
    ...
    if (action === "start") {
      ...
      await db.update(rooms).set({
        ...
        lastBidAt: null, // No bid yet, timer doesn't start until first bid
    ...
    if (action === "bid") {
      ...
      // Check if 15s countdown already expired (someone else should have closed it)
      if (r.lastBidAt) {
        const elapsed = now - r.lastBidAt;
        if (elapsed > 18000) {
          // 15s + 3s grace period passed, auction should have auto-closed
      ...
      history.push({ playerId, playerName: b.name, amount, timestamp: now });

      await db.update(rooms).set({
        currentHighestBid: amount,
        currentHighestBidder: playerId,
        bidHistory: history,
        lastBidAt: now, // Reset the 3s countdown
      }).where(eq(rooms.id, roomId));
    ...
    if (action === "close") {
      // Can be called by host OR by auto-close from client
      if (!r.auctionActive) {
        return NextResponse.json({ error: "No active auction" }, { status: 400 });
      }
      ...
      if (r.currentHighestBidder && auctionedPlayer) {
        ...
        if (winner.length > 0) {
          ...
          // SOLD: mark this combo so it never rolls again
          soldCombos.push(`${pos}-${optIdx}`);
        }
      }
      // If nobody bid, combo is NOT added to soldCombos (it can be rolled again)
    ...
    if (action === "skip") {
      ...
      // Skip does NOT add to auctionedPositions, so it can roll again
      await db.update(rooms).set({
        auctionActive: false,
        currentHighestBidder: null,
        currentHighestBid: null,
