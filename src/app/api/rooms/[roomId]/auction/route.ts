...
// POST - Start auction with a price, or place a bid
export async function POST(
  ...
  try {
    ...
    if (action === "start") {
      // Host starts the auction with a starting price
    ...
    if (action === "bid") {
      ...
      // Check budget - must have enough to not go bankrupt
      // They need to still have money for remaining positions
      const filledCount = Object.keys(lineup).length;
      const remainingPositions = 11 - filledCount - 1; // -1 for the current one being bid on
      // Minimum reserve: can't bid more than budget minus 0 (just can't go below 0)
      if (amount > b.budget) {
    ...
    if (action === "close") {
      ...
      // Mark this position-option as auctioned
      const auctioned = [...((r.auctionedPositions as string[]) || []), `${pos}-${optIdx}`];
    ...
    if (action === "skip") {
      // Host skips (no bids) - mark as auctioned but nobody gets it
      const host = await db.select().from(players).where(eq(players.id, playerId)).limit(1);
      if (host.length === 0 || !host[0].isHost) {
